 ...
 * Bypasses string parsing for maximum I/O speed.
 */
...
// 2. CORE MATHEMATICS (LOW-LATENCY SIMD OPERATIONS)
...
void linear_layer(const vector<float> &W, const vector<float> &X, const vector<float> &b, vector<float> &Z, int rows, int cols) {
    ...
    vecZ.noalias() = matW * vecX + vecB; // AVX2 Vectorized multiplication
}

// 3. EXECUTION ENGINE (TCP DAEMON & SINGLE CLI MODE)

int main(int argc, char* argv[]) {
    bool is_daemon_mode = (argc == 1); 
    
    if (!is_daemon_mode && argc != 7) {
    ...
    // 1. Load Neural Network Weights (BINARY FORMAT)
    ...
    // PATH A : TCP DAEMON MODE (FOR STREAMLIT)
    if (is_daemon_mode) {
        ...
        address.sin_port = htons(5555); // Port 5555

        if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) < 0) return 1;
        if (listen(server_fd, 3) < 0) return 1;

        cout << "[C++ Engine] TCP Daemon listening on local port 5555..." << endl;
        ...
        while(true) {
            ...
            // Send results back to Python via TCP
