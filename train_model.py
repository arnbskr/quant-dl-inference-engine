...
# Modélisation du smile de volatilité (dépendance à la moneyness S/K et au temps)
moneyness = S / K
sigma_smile = 0.20 + 0.5 * (np.log(moneyness))**2 + 0.1 * T
sigma_smile = np.clip(sigma_smile, 0.05, 0.90) # Borne de sécurité
...
# 5. Évaluation du modèle sur les données de test (Out-of-Sample)
model.eval()
with torch.no_grad():
...
# 6. Export des poids du modèle et du scaler pour utilisation en C++
os.makedirs("model_weights", exist_ok=True)
for name, param in model.named_parameters():
    np.savetxt(f"model_weights/{name}.csv", param.detach().numpy(), delimiter=",")

# Export du scaler (moyenne et écart-type) pour la normalisation des données en C++
...
print("\nPoids et Scaler exportés avec succès dans 'model_weights/'.")